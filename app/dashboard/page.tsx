...
export default async function DashboardPage() {
  ...
  if (edgeError) {
    return (
      <div className="p-10">
        <h1 className="text-3xl font-bold text-red-600 mb-4">Diagnostic Error</h1>
        <pre className="bg-slate-900 text-green-400 p-6 rounded-xl text-sm overflow-auto">{edgeError}</pre>
      </div>
    )
  }
                ...
                <div className="flex items-start justify-between gap-6">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-6 text-sm font-medium text-slate-600 mt-2">
                      ...
                      <span>{profile.frequency} sync</span>
                      {profile.last_synced_at && (
                        <span>Last synced: {format(new Date(profile.last_synced_at), 'MMM d, yyyy')}</span>
                      )}
                    </div>
                  </div>
